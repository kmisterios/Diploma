...
def link():
    ...
    schema = schemass[idCrawl]
    try:
        with open('collection' + str(idCrawl + 1) + '.json', 'r') as f:
            samples = json.loads(f.read())
    except:
        samp = noErrors(n, schema)
        with open('noAnomalies_id_' + str(idCrawl + 1) + '.json', 'r') as f:
            collection_init = json.loads(f.read())
        with open('collection' + str(idCrawl + 1) + '.json', 'w') as json_file:
    ...
    if request.method == 'POST':
        ...
        if num_err == 0:
            with open('collection' + str(idCrawl + 1) + '.json', 'w') as f:
                json.dump(samples, f)
...
def verify_test(schema_id):
    ...
    schema = schemass[schema_id - 1]
    try:
        with open('collection' + str(schema_id) + '.json', 'r') as f:
            samples_collection = json.loads(f.read())
    except:
        sampi = noErrors(n, schema)
        with open('noAnomalies_id_' + str(schema_id) + '.json', 'r') as f:
            samples_collection = json.loads(f.read())
        with open('collection' + str(schema_id) + '.json', 'w') as json_file:
    ...
    if type_of_error == '0':
        ...
        with open('anomaly' + type_of_error + '_id_' + str(schema_id) + '.json', 'r') as f:
            new_samples = json.loads(f.read())
    ...
    for sample in new_samples:
        try:
            ...
            num_err += 1
            return {"sample": sample, "message": 'Error in schema'}
    # if there are errors in schemas, it's no reason to continue tests
    if num_err == 0:
    ...
    if len(result_item_count) > 2:
        for j in range(len(result_item_count) - 1):
            length = result_item_count[j] - len(samples_collection) if j == 0 else result_item_count[j] - result_item_count[j - 1]
            for k in range(length):
                if j % 2 == 0:
                    samp.append(0)
                else:
    ...
    for i in range(len(new_samples)):
        samp.append(0)
    with open('collection' + str(schema_id) + '.json', 'w') as f:
        json.dump(samples, f)
    return {"prediction": samp, "message": message}
